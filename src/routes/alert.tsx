  ...
  return (
    <div className="relative flex min-h-screen flex-col bg-background">
      ...
      <div className="relative z-10 rounded-t-3xl border-t border-destructive/30 bg-card px-6 pb-8 pt-6 shadow-2xl animate-fade-in">
        ...
        <Link to="/dashboard" className="absolute right-4 top-4 rounded-full p-2 text-muted-foreground hover:bg-secondary active:scale-90 transition-all">
          <X className="h-5 w-5" />
        </Link>
        ...
        <div className="mt-6 rounded-xl border border-destructive/20 bg-destructive/5 p-4">
          ...
          <div className="mt-2 h-2 overflow-hidden rounded-full bg-secondary">
            <div className="h-full rounded-full bg-destructive transition-all duration-1000" style={{ width: "78%" }} />
          </div>
        ...
        <div className="mt-4 rounded-xl border border-primary/30 bg-primary/5 p-4">
          ...
          <button
            onClick={() => toast.success("¡Misión de calma iniciada! Respira profundo...", { icon: "🧘", duration: 5000 })}
            className="mt-3 w-full rounded-lg bg-primary py-2.5 text-sm font-semibold text-primary-foreground transition-all hover:shadow-lg hover:shadow-primary/30 active:scale-95"
          >
        ...
        <div className="mt-4 flex gap-3">
          <button
            onClick={() => toast("Llamando a línea de apoyo...", { icon: "📞" })}
            className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-border bg-card py-3 text-sm text-foreground transition-all hover:bg-secondary active:scale-95"
          >
            <Phone className="h-4 w-4" /> Llamar
          </button>
          <button
            onClick={() => toast("Abriendo chat de apoyo...", { icon: "💬" })}
            className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-border bg-card py-3 text-sm text-foreground transition-all hover:bg-secondary active:scale-95"
          >
